...
class Blockchain:
    ...
    def create_block(self, proof, previous_hash):
        block = {'index': len(self.chain) + 1,
                 ...
                 'transacations': self.transactions}
        self.transactions = []
    ...
    def add_transactions(self, sender, amount):
...
def replace_chain():
    is chain_replaced = blockchain.replace_chain()
    if is valid:
        response = {'message': 'The nodes had different chains so the chain was replaced by the longest one.', 'new chain': blockchain.chain}
    else:
